...
public class BNLJTraceGenerator extends PageReferenceTraceGenerator
{
	 ...
	 */
	public PageReferenceTrace generateBNLJ(long transactionNumber, String fileNameOuter, int pageCountOuter, String fileNameInner, int pageCountInner, int groupSize)
	{
		...
		{
			List<PageId> outerBlock = generateSequentialPages(fileNameOuter, offset, Math.min(offset + groupSize, pageCountOuter));
			
			ret.concatenate(build(new TransactionId(transactionNumber), outerBlock,PageReferenceType.REQUEST));
			ret.concatenate(buildRequestAndRelease(new TransactionId(transactionNumber), generateSequentialPages(fileNameInner,0, pageCountInner)));
