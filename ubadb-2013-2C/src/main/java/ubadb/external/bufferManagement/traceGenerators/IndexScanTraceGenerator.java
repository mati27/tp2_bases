...
{
	public PageReferenceTrace generateIndexScanClustered(long transactionNumber, String fileName, int indexHeight, int referenceStart, int referenceCount)
	{
		List<PageId> indexPages = generateSequentialPages(fileName + "_index", 0, indexHeight);
		List<PageId> filePages = generateSequentialPages(fileName, referenceStart, referenceStart+referenceCount);
	 ...
	 */
	public PageReferenceTrace generateIndexScanUnclusteredForASingleLeaf(long transactionNumber, String fileName, int indexHeight, int referenceCount, int filePageCount)
	{
		List<PageId> indexPages = generateSequentialPages(fileName + "_index", 0, indexHeight);
		List<PageId> filePages = generateRandomPages(fileName, referenceCount, filePageCount);
