...
public class PageReferenceTraceSerializer
{
	...
	private String convertPageReferenceToString(PageReference reference)
	{
		...
		ret += reference.getPageId().getTableId().getRelativeFilePath() + TABLE_SEPARATOR;
	...
	private PageReference convertPageReferenceFromString(String line)
	{
		...
		String tableId = tokenizer3.nextToken();
		String pageNumber = tokenizer3.nextToken();
		
		return new PageReference(new TransactionId(Long.valueOf(transactionId)), new PageId(Integer.valueOf(pageNumber), new TableId(tableId)), PageReferenceType.valueOf(type));
	}
}
